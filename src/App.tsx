...
export default function App(){
  ...
  useEffect(()=>{if(!session?.user||session.user.scope==='bench')return;void refresh();const interval=setInterval(()=>{if(!document.hidden&&!busyRef.current)void refresh();},5000);const visible=()=>{if(!document.hidden)void refresh();};const offline=()=>setOnline(false);document.addEventListener('visibilitychange',visible);window.addEventListener('online',visible);window.addEventListener('offline',offline);return()=>{clearInterval(interval);document.removeEventListener('visibilitychange',visible);window.removeEventListener('online',visible);window.removeEventListener('offline',offline);};},[session?.user?.email,session?.user?.scope,refresh]);
  ...
  const signedOut=useCallback(()=>{if(sessionRef.current)acceptSession({...sessionRef.current,user:null});},[acceptSession]);
  if(session&&(!session.user||authEntry.startsWith('#auth')))return <Auth key={authEntry} session={session} entry={authEntry} onSession={next=>{setAuthEntry('');acceptSession(next);}}/>;
