...
import { CalendarDays, ChartNoAxesColumnIncreasing, CheckCircle2, LayoutDashboard, LoaderCircle, LogOut, Radio, RefreshCw, Settings2, ShieldCheck, Users, Wallet, WifiOff, X } from 'lucide-react';
...
import { subscribeSessionChange } from './sessionEvents';
...
export default function App(){
  const [authEntry,setAuthEntry]=useState(()=>location.hash);
  useEffect(()=>{const changed=()=>setAuthEntry(location.hash);window.addEventListener('hashchange',changed);return()=>window.removeEventListener('hashchange',changed);},[]);
  ...
  const emailSessionClose=useRef<{entry:string;promise:Promise<unknown>}|null>(null);
  const clearWorkspace=useCallback(()=>{epoch.current++;refreshToken.current=null;workspaceRef.current=null;setWorkspace(null);lastSample.current=0;busyRef.current=false;setBusy(false);setDialog(null);setMatchId(null);setView('dashboard');setToast(null);clearTimeout(toastTimer.current);},[]);
  const acceptSession=useCallback((next:Session)=>{if(sessionRef.current?.user?.id!==next.user?.id||sessionRef.current?.user?.scope!==next.user?.scope)clearWorkspace();sessionRef.current=next;setSession(next);},[clearWorkspace]);
  ...
  const refresh=useCallback(async()=>{
    if(refreshToken.current||authEntry.startsWith('#auth')||!sessionRef.current?.user||sessionRef.current.user.scope==='bench')return;
  ...
  },[receive,acceptSession,authEntry]);
  const bootstrap=useCallback(async(clear=true)=>{
    const request=++sessionRequest.current;
    if(clear){clearWorkspace();sessionRef.current=null;setSession(null);}
    const generation=epoch.current;
    setBootError('');
    try{
      const emailLink=['verify','reset'].includes(new URLSearchParams(authEntry.replace(/^#/,'')).get('auth')||'');
      if(emailLink){
        // Every session check waits for the same logout, including focus events.
        if(emailSessionClose.current?.entry!==authEntry){
          const closing={entry:authEntry,promise:api('logout',{})};
          emailSessionClose.current=closing;
          closing.promise=closing.promise.catch(error=>{if(emailSessionClose.current===closing)emailSessionClose.current=null;throw error;});
        }
        await emailSessionClose.current.promise;
      }else emailSessionClose.current=null;
      const next:Session=await api('session');
      if(request!==sessionRequest.current||generation!==epoch.current)return;
      acceptSession(authEntry.startsWith('#auth')?{...next,user:null}:next);
    }catch{
      if(request!==sessionRequest.current||generation!==epoch.current)return;
      clearWorkspace();sessionRef.current=null;setSession(null);
      setBootError('No se puede conectar con la app. Vuelve a intentarlo.');
    }
  },[acceptSession,clearWorkspace,authEntry]);
  useEffect(()=>{void bootstrap();return()=>{sessionRequest.current++;epoch.current++;};},[bootstrap]);
  useEffect(()=>subscribeSessionChange(()=>{void bootstrap();}),[bootstrap]);
  useEffect(()=>{
    // Recheck on return too: OAuth redirects and suspended tabs can miss a broadcast.
    const visible=()=>{if(!document.hidden)void bootstrap(false);};
    window.addEventListener('focus',visible);document.addEventListener('visibilitychange',visible);
    return()=>{window.removeEventListener('focus',visible);document.removeEventListener('visibilitychange',visible);};
  },[bootstrap]);
  useEffect(()=>{if(authEntry.startsWith('#auth')||!session?.user||session.user.scope==='bench')return;void refresh();const interval=setInterval(()=>{if(!document.hidden&&!busyRef.current)void refresh();},5000);const visible=()=>{if(!document.hidden)void refresh();};const offline=()=>setOnline(false);window.addEventListener('online',visible);window.addEventListener('offline',offline);return()=>{clearInterval(interval);window.removeEventListener('online',visible);window.removeEventListener('offline',offline);};},[authEntry,session?.user?.id,session?.user?.scope,refresh]);
  ...
  async function command(type:string,payload:Record<string,unknown>){
    ...
    try{let data;try{data=await api('command',body);}catch(e){if(generation!==epoch.current)return false;if(e instanceof TypeError&&navigator.onLine)data=await api('command',body);else throw e;}if(generation!==epoch.current)return false;receive(data);if(!type.startsWith('match.'))notify('Cambios guardados.');return true;}
    catch(e){if(generation!==epoch.current)return false;const err=e as Error & {status?:number};notify(err.message||'No se pudo guardar el cambio.',true);await refresh();return false;}
  ...
  async function logout(){if(session?.demo){notify('La demo usa datos de ejemplo. En el despliegue podrás crear tu cuenta.');return;}const generation=epoch.current;try{await api('logout',{});if(generation===epoch.current&&sessionRef.current)acceptSession({...sessionRef.current,user:null});}catch(e){if(generation===epoch.current)notify((e as Error).message,true);}}
  const renderEpoch=epoch.current;
  const signedOut=useCallback(()=>{if(renderEpoch===epoch.current&&sessionRef.current)acceptSession({...sessionRef.current,user:null});},[acceptSession,renderEpoch]);
  const benchStarted=useCallback(()=>{if(renderEpoch===epoch.current)void bootstrap();},[bootstrap,renderEpoch]);
  const updated=useCallback((data:Parameters<typeof receive>[0])=>{if(renderEpoch===epoch.current)receive(data);},[receive,renderEpoch]);
  if(session&&(!session.user||authEntry.startsWith('#auth')))return <Auth key={authEntry} session={session} entry={authEntry} onSession={next=>{if(renderEpoch!==epoch.current)return;setAuthEntry('');acceptSession(next);}}/>;
  if(session?.user?.scope==='bench')return <Bench onSignedOut={signedOut}/>;
