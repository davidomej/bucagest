...
import { FixtureForm, ImportFixtures, PlayerForm, SettingsPage } from './forms';
import { Crest, Logo, Modal } from './components';
import { api, initials } from './lib';
import type { Match, Player, Session, Team } from './types';
import { registerMinutesTool } from './webmcp';
type Dialog = {kind:'player';player?:Player}|{kind:'fixture';match?:Match}|{kind:'import'}|{kind:'archive';player:Player}|{kind:'delete';match:Match}|null;
export default function App(){
  const [session,setSession]=useState<Session|null>(null),[team,setTeam]=useState<Team|null>(null),[bootError,setBootError]=useState(''),[view,setView]=useState('dashboard');
  ...
  const teamRef=useRef<Team|null>(null),busyRef=useRef(false),clockRef=useRef({server:Date.now(),local:performance.now()}),toastTimer=useRef<ReturnType<typeof setTimeout>|undefined>(undefined);
  const sessionRef=useRef<Session|null>(null),epoch=useRef(0),refreshToken=useRef<symbol|null>(null),lastSample=useRef(0);
  const acceptSession=useCallback((next:Session)=>{if(sessionRef.current?.user?.id!==next.user?.id){epoch.current++;teamRef.current=null;setTeam(null);lastSample.current=0;setDialog(null);setMatchId(null);setView('dashboard');}sessionRef.current=next;setSession(next);},[]);
  ...
  const receive=useCallback((data:{team:Team;userId:string;serverNow:number})=>{
    ...
    if(previous?.id===data.team.id&&(data.team.revision<previous.revision||(data.team.revision===previous.revision&&data.serverNow<lastSample.current)))return;
    teamRef.current=data.team;setTeam(data.team);lastSample.current=data.serverNow;clockRef.current={server:data.serverNow,local:performance.now()};setNow(data.serverNow);setOnline(true);setBootError('');
  },[acceptSession]);
  const refresh=useCallback(async()=>{
    ...
    catch(e){if(generation!==epoch.current)return;if((e as {status?:number}).status===401){if(sessionRef.current)acceptSession({...sessionRef.current,user:null});}else{setOnline(false);if(!teamRef.current)setBootError('No se puede cargar el equipo. Comprueba la conexión y vuelve a intentarlo.');}}
  ...
  useEffect(()=>registerMinutesTool(()=>teamRef.current),[]);
  async function command(type:string,payload:Record<string,unknown>){
    if(busyRef.current||!teamRef.current)return false;
    busyRef.current=true;setBusy(true);
    const generation=epoch.current;
    const body={type,payload,teamId:teamRef.current.id,revision:teamRef.current.revision,operationId:crypto.randomUUID()};
  ...
  if(!team)return <div className="loading-screen"><Logo/>{bootError?<><p role="alert">{bootError}</p><button className="button" onClick={()=>session?.user?refresh():bootstrap()}><RefreshCw size={17}/> Reintentar</button></>:<LoaderCircle className="spin"/>}</div>;
