...
export function createApp(store, { production = false, origin = process.env.APP_ORIGIN, registrationAllowed = process.env.ALLOW_REGISTRATION !== 'false' } = {}) {
  ...
  app.use('/api',(req,res,next)=>{
    if(!(req.path==='/assets' && ['GET','HEAD'].includes(req.method))) res.set('Cache-Control','no-store');
  ...
  app.get('/api/assets/:id',async(req,res)=>{
    ...
    res.set('Cache-Control','private, max-age=31536000, immutable').type(asset.mime).send(asset.bytes);
  });
